...
class SampleVerticleTest {
    ...
    class CustomLifecycleTest {
        ...
        @BeforeEach
        void prepare() {
            this.vertx = Vertx.vertx(new VertxOptions().setMaxEventLoopExecuteTime(1000).setPreferNativeTransport(true)
        ...
        void deploySampleVerticle(VertxTestContext testContext) {
            this.vertx.deployVerticle(new SampleVerticle(), testContext.succeeding(id -> testContext.completeNow()));
        }
        ...
        void httpRequest(VertxTestContext testContext) {
            ...
            this.vertx.deployVerticle(new SampleVerticle(), testContext.succeeding(id -> {
                webClient.get(11981, "localhost", "/yo").as(BodyCodec.string()).send(testContext.succeeding(resp -> {
                    testContext.verify(() -> {
                        assertThat(resp.statusCode()).isEqualTo(200);
                        assertThat(resp.body()).contains("Yo!");
                        testContext.completeNow();
